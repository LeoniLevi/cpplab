...
// ?????
void mymergesort1(int arr[], size_t first, size_t last, int temp[]);
...
void my_merge_sort_arr(int items[], size_t numItems)
{
	...
	int* temp = varr.data();
	mymergesort1(items, 0, numItems - 1, temp);
}
//-------------------
...
void mymergesort1(int arr[], size_t first, size_t last, int temp[])
{
	...
	if (first < last) {
		...
		mymergesort1(arr, first, mid, temp);
		mymergesort1(arr, mid + 1, last, temp);
