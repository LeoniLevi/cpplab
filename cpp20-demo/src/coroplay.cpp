...
bool play_fibonacci()
{
    try {
        uint64_t fib_num;
        auto gen = fibonacci_sequence(15); //max 94 before uint64_t overflows
        for (int j = 0; gen; j++) {
            fib_num = gen();
            std::cout << "fib(" << j << ")=" << fib_num << '\n';
        }
    }
...
struct ReturnObject {
    struct promise_type {
        ...
        std::suspend_never final_suspend() noexcept { return {}; }     //Note(2) 
        void unhandled_exception() {}
        void return_void() noexcept {}
    };
};
ReturnObject my_suspend_func() {
    std::cout << "1. hello from coroutine";
    co_await std::suspend_always{}; //Note(3)
    std::cout << "2. hello from coroutine";
}
//call foo
int play_simple_suspend() 
{ 
    my_suspend_func();
    return 0;
}
...
{
    puts("\n === C++20 <COROUTINES> working example...");
    puts("---------------------");

    puts("\n-- play_simple_suspend<coro>..");
    play_simple_suspend();
